...
- `docs/`: 配信用静的 Web 出力
- `docs/story-data.js`: Web Reader が読む本文・章構造・タイトル情報
- `docs/images/title/`: 配信用タイトル画像
- `docs/images/characters/`: 配信用キャラクター画像
- `docs/images/chapters/`: 配信用章扉絵
- `docs/images/ui/`: 配信用 UI 素材
...
5. 必要なら `generate-title-image.mjs` でタイトル画像を作る
6. キャラクター画像と章扉絵を生成する
7. `build-web-novel.mjs` で `docs/` をビルドする
...
- `raw` やメタデータ JSON は中間生成物として扱い、公開成果物の基準にはしない
